...
def get_help_strings(binary: str) -> Dict[str, str]:
    ...
    with subprocess.Popen([binary, "--help=warnings"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL) as proc:
...
def get_all_options(binary: str) -> List[str]:
    ...
    with subprocess.Popen([binary, "-Q", "--help=warnings"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL) as proc:
        output = proc.stdout.read().decode("utf-8")
